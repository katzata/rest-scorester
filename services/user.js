...
async function register(req, res) {
    ...
    req.db.getEntry({ users: { username: req.body.username }}).then(queryRes => {
...
async function login(req, res) {
    ...
    req.db.getEntry(query).then(queryRes => {
        ...
        if (user && user.id) {
            ...
            const errorMessage = user && user.error ? user.error : { code: 409, list: ["Wrong username or password"]};
...
async function logout(req, res) {
    ...
    req.db.updateEntry(id, updateData).then(updateRes => {
        ...
        if (Number(updateRes.affectedRows) > 0) {
            setResponseHeaders(req, res, { accessToken: "clear", refreshToken: "clear" })
                .status(201)
                .send(JSON.stringify({ status: true }));
        } else {
            setResponseHeaders(req, res, { accessToken: "clear", refreshToken: "clear" })
                .status(401)
                .send(JSON.stringify({ status: false }));
        };
    });
};
...
async function deleteAccount(req, res) {
    ...
    req.db.deleteEntry(id, updateData).then(updateRes => {
        if (updateRes[0] && updateRes[0].error) return catchDbErrors(req, updateRes[0].error)
...
async function checkIfLogged(req, res) {
    ...
    if (id !== "undefined" && req.cookies.rToken !== "undefined") {
        req.db.getEntry({ users: { id }}).then(queryRes => {
            ...
            if (rToken) {
                ...
                if (tokenData) {
                    ...
                    setResponseHeaders(req, res)
                        .status(200)
                        .send(JSON.stringify({ Errors: ["jwt expired"] }));
                };
            } else {
                setResponseHeaders(req, res)
                    .status(200)
                    .send(JSON.stringify({ Errors: ["jwt error"] }));
            };
        });
    } else {
        setResponseHeaders(req, res)
            .status(200)
            .send(JSON.stringify({ Errors: ["no id"] }));
    };
};
        ...
        req.db.getEntry(query).then(getRes => {
            ...
            if (userData) {
                ...
                req.db.updateEntry(id, updatedData).then(updateRes => {
                    const { changedRows, message, warningCount } = updateRes;
                    setResponseHeaders(req, res).send(JSON.stringify({ changedRows, message, warningCount }));
                });
            };
        });
    } else {
        setResponseHeaders(req, res).send(JSON.stringify({ res: "yay" }));
    };
};
...
module.exports = () => (req, res, next) => {
    req.user = {
        ...
        delete: (...params) => deleteAccount(req, res, ...params)
    };
